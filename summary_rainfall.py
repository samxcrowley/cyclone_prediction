...
precip_density_obs = xr.DataArray(np.zeros((len(latitudes), len(longitudes))),
                                    coords=[latitudes, longitudes],
                                    dims=["lat", "lon"])
precip_density_pred = xr.DataArray(np.zeros((len(latitudes), len(longitudes))),
                                    coords=[latitudes, longitudes],
                                    dims=["lat", "lon"])
...
for name in tc_names:
    ...
    ds = None
    try:
        ds = xr.open_dataset(f"/scratch/ll44/sc6160/out/rainfall/{name}_rainfall.nc")
    except:
        print(f"No rainfall data for {name}")
        continue

    precip_density_obs = precip_density_obs + ds['obs']
    precip_density_pred = precip_density_pred + ds['pred']
...
max_value = max(np.max(precip_density_obs), np.max(precip_density_pred))
if max_value == 0:
    max_value = 1  # fallback if both datasets are empty

# IMERG
im1 = ax1.pcolormesh(longitudes, latitudes, precip_density_obs, 
...
# GraphCast
im2 = ax2.pcolormesh(longitudes, latitudes, precip_density_pred, 
                     shading='auto', cmap='Blues', vmin=0, vmax=max_value,
                     transform=proj)
...
# difference
precip_density_difference = precip_density_obs - precip_density_pred
...
if args.good:
    plt.savefig(f"/scratch/ll44/sc6160/out/plots/summary/summary_rainfall_good.png", bbox_inches='tight', pad_inches=0.1)
else:
    plt.savefig(f"/scratch/ll44/sc6160/out/plots/summary/summary_rainfall_all.png", bbox_inches='tight', pad_inches=0.1)